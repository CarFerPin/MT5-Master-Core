...
void MC_Reset()
{
   g_masterControl.global_switch.enabled = true;
...
{
   string globalObj;
   if(!MC_FindSection(g_masterControlRawJson, "global_switch", globalObj))
      return;

   bool enabled;
   if(MC_ExtractJsonBool(globalObj, "enabled", enabled))
      g_masterControl.global_switch.enabled = enabled;
}
...
void MC_ParseSymbol()
{
   ...
   string symbolObj;
   if(!MC_FindSection(symbolsObj, symbolKey, symbolObj))
      return;
   ...
   parsed.enabled = true;
   parsed.mode = "trend";
   parsed.risk.base_volume = 0.0;
   parsed.risk.counter_multiplier = 1.0;
...
bool IsEnabled(string &reason)
{
   // global kill switch
   if(!g_masterControl.global_switch.enabled)
   {
      reason = "GLOBAL DISABLED";
      return false;
   }

   // símbolo no definido en JSON
   ...
   // símbolo deshabilitado explícitamente
   if(!g_symbolControl.enabled)
   {
...
{
   if(g_hasSymbolOverride)
      return g_symbolControl.risk.base_volume;
