// MQL5\Include\Vturn_Mom\Vturn_NotificationsTelegram.mqh
#ifndef VTURN_MOM_NOTIFICATIONS_TELEGRAM_MQH
#define VTURN_MOM_NOTIFICATIONS_TELEGRAM_MQH

#include <Vturn_Mom/Vturn_Telegram.mqh> 

// --- CLOSE-by-position tracking (se quedan igual)
...
bool TG_ClosedNetByPosId(const ulong pos_id,
                         const ulong magic,
                         double &net,
...
// --- Telegram helpers (sin globals)
void TG_Send(THREETelegram &tg, const bool enabled, const string msg)
{
   if(!enabled) return;
...
void TG_SendForce(THREETelegram &tg, const bool enabled, const string msg)
{
   if(!enabled) return;
