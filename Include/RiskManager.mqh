...
{
   return "RM_DAY_START_" + AccountInfoString(ACCOUNT_SERVER);
}
...
void RM_UpdateRuntime()
{
   ...
   if(RM_DayKey(now) != RM_DayKey(g_last_day))
   {
      g_equity_day_start = eq;
      g_last_day = now;
      string key = RM_KeyDayStart();
      GlobalVariableSet(key, g_equity_day_start);
      GlobalVariableSet(key + "_DAY", (double)RM_DayKey(now));
      g_risk_logged_daily = false;
      g_risk_logged_block = false;
   }
...
{
   if(g_equity_day_start <= 0.0) return 0.0;
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
